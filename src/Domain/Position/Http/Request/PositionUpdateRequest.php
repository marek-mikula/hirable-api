...
class PositionUpdateRequest extends AuthRequest
{
    ...
    public function rules(
        ...
        return [
            ...
            'educationField' => [
                Rule::excludeIf(!in_array('educationField', $keys)),
                'nullable',
                'string',
                'max:255',
            ],
            ...
            'experienceWeight' => [
                Rule::excludeIf(!in_array('experienceWeight', $keys)),
                'required',
                'integer',
                'min:0',
                'max:100',
            ...
            'educationWeight' => [
                Rule::excludeIf(!in_array('educationWeight', $keys)),
                'required',
                'integer',
                'min:0',
                'max:100',
    ...
    public function toData(): PositionUpdateData
    {
        ...
        return PositionUpdateData::from([
            ...
            'educationField' => in_array('educationField', $keys) ? ($this->filled('educationField') ? (string) $this->input('educationField') : null) : null,
            'seniority' => in_array('seniority', $keys) ? ($this->collect('seniority')->map(fn (mixed $val): string => (string) $val)->all()) : [],
            ...
            'workloads' => in_array('workloads', $keys) ? ($this->collect('workloads')->map(fn (mixed $val): string => (string) $val)->all()) : [],
            'employmentRelationships' => in_array('employmentRelationships', $keys) ? ($this->collect('employmentRelationships')->map(fn (mixed $val): string => (string) $val)->all()) : [],
            'employmentForms' => in_array('employmentForms', $keys) ? ($this->collect('employmentForms')->map(fn (mixed $val): string => (string) $val)->all()) : [],
            'benefits' => in_array('benefits', $keys) ? ($this->collect('benefits')->map(fn (mixed $val): string => (string) $val)->all()) : [],
            'files' => in_array('files', $keys) ? ($this->file('files', [])) : [],
            'languageRequirements' => in_array('languageRequirements', $keys) ? $this->collect('languageRequirements')->map(fn (array $item): \Domain\Position\Http\Request\Data\LanguageRequirementData => LanguageRequirementData::from([
                'language' => (string) $item['language'],
                'level' => (string) $item['level'],
            ])) : [],
            'hiringManagers' => in_array('hiringManagers', $keys) ? ($this->collect('hiringManagers')->map(fn (mixed $value): int => (int) $value)->all()) : [],
            'recruiters' => in_array('recruiters', $keys) ? ($this->collect('recruiters')->map(fn (mixed $value): int => (int) $value)->all()) : [],
            'approvers' => in_array('approvers', $keys) ? ($this->collect('approvers')->map(fn (mixed $value): int => (int) $value)->all()) : [],
            'externalApprovers' => in_array('externalApprovers', $keys) ? ($this->collect('externalApprovers')->map(fn (mixed $value): int => (int) $value)->all()) : [],
            ...
            'experienceWeight' => in_array('experienceWeight', $keys) ? ((int) $this->input('experienceWeight')) : null,
            ...
            'tags' => in_array('tags', $keys) ? ($this->collect('tags')->map(fn (mixed $val): string => (string) $val)->all()) : [],
        ]);
    }
}
