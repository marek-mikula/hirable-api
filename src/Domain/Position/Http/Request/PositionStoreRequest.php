...
class PositionStoreRequest extends AuthRequest
{
    ...
    public function toData(): PositionStoreData
    {
        return PositionStoreData::from([
            ...
            'seniority' => $this->collect('seniority')->map(fn (mixed $val): string => (string) $val)->all(),
            ...
            'workloads' => $this->collect('workloads')->map(fn (mixed $val): string => (string) $val)->all(),
            'employmentRelationships' => $this->collect('employmentRelationships')->map(fn (mixed $val): string => (string) $val)->all(),
            'employmentForms' => $this->collect('employmentForms')->map(fn (mixed $val): string => (string) $val)->all(),
            'benefits' => $this->collect('benefits')->map(fn (mixed $val): string => (string) $val)->all(),
            'files' => $this->file('files', []),
            'languageRequirements' => $this->collect('languageRequirements')->map(fn (array $item): \Domain\Position\Http\Request\Data\LanguageRequirementData => LanguageRequirementData::from([
                'language' => (string) $item['language'],
                'level' => (string) $item['level'],
            ])),
            'hiringManagers' => $this->collect('hiringManagers')->map(fn (mixed $value): int => (int) $value)->all(),
            'recruiters' => $this->collect('recruiters')->map(fn (mixed $value): int => (int) $value)->all(),
            'approvers' => $this->collect('approvers')->map(fn (mixed $value): int => (int) $value)->all(),
            'externalApprovers' => $this->collect('externalApprovers')->map(fn (mixed $value): int => (int) $value)->all(),
            ...
            'tags' => $this->collect('tags')->map(fn (mixed $tag): string => (string) $tag)->all(),
        ]);
    }
}
